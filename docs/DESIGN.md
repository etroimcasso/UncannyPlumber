...
| Rendering | Shipped engine surface only; zero engine work items (`renderer-capability-audit.md`, 2026-08-28). The status bar, the window and the playfield are separate tile layers over the same cell data, bounded by `TileWrap::Blank` and — for the one clip not on a tile edge, the game-over banner — a `stencil` region. Every tile bitmap the original rewrites at runtime (the animated tile, the disintegration masks) ships as pre-baked variants in the extracted sheets, selected per cell. The engine's sub-tick interpolation is **off**: the original moves the camera and every sprite by whole pixels per frame and never shows a fractional position. |
